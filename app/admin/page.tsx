...
export default async function Page(){
 ...
 const metrics:[string,number,string,DashboardMetricIconName][]=[['Active Students',activeStudents,'Enrolled accounts','students'],['Pending Review',count('pending'),'Needs a decision','review'],['Completed Requests',completed,`${total} total requests`,'completed'],['Available Seats',availableSeats,`${openSchedules.length} open schedules`,'seats']]
 return <div className="space-y-7 page-stack">
  <section className="page-heading"><p className="eyebrow">CLINICAL HEAD DASHBOARD</p><h1 className="dashboardGreeting">Welcome back, {profile.first_name}.</h1><p className="muted text-sm">Monitor student progress, review demand, and keep duty capacity available.</p></section>
  {error&&<p className="notice" role="alert">Unable to load all analytics. Check the database setup.</p>}
  <section className="analytics-kpis" aria-label="Clinical Head analytics">{metrics.map(([label,value,detail,icon])=><article className="dashboardCard analytics-kpi" key={label}><div><span className="countIcon" data-icon={icon}><DashboardMetricIcon name={icon}/></span><span className="badge">{label}</span></div><strong>{value}</strong><small>{detail}</small></article>)}</section>
