...
export async function postAnnouncement(form:FormData) {
 ...
 if(operation!=='delete'&&batch){try{await getBatchRule(batch)}catch{redirect('/admin/announcements?error=Choose+an+active+batch')}}
 if(operation!=='delete'&&(!title||title.length>150||!content||content.length>10000)) redirect('/admin/announcements?error=Invalid+announcement')
