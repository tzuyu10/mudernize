...
function Fields({announcement}:{announcement?:any}){
 return <>
...
export default async function Page({searchParams}:{searchParams:{error?:string;message?:string}}){
 ...
 const {data,error}=await getAllAnnouncements()
 return <div className="space-y-6">
  ...
  <ResetAfterSubmitForm action={postAnnouncement} className="dashboardCard p-6 space-y-4"><h2 className="font-semibold">New Announcement</h2><Fields/><ConfirmButton className="primary" message="Publish this announcement?">Publish Announcement</ConfirmButton></ResetAfterSubmitForm>
  <div className="space-y-3">{data?.map(announcement=>{
   ...
   return <details key={announcement.announcement_id} className="dashboardCard p-5">
    ...
    <ResetAfterSubmitForm id={editFormId} action={postAnnouncement} className="space-y-4 mt-4"><input type="hidden" name="announcement_id" value={announcement.announcement_id}/><Fields announcement={announcement}/></ResetAfterSubmitForm>
