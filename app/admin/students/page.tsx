...
export default async function Page({searchParams:searchParamsPromise}:{searchParams:Promise<{batch?:string;q?:string;error?:string;message?:string;created?:string}>}){
 ...
 const [{data:batchConfigs},{data:students,error},{data:registrations},{data:adjustments,error:adjustmentError}]=await Promise.all([
 ...
 const rows=(students||[]).filter(student=>(!searchParams.batch||student.batch===searchParams.batch)&&(!searchParams.q||[student.first_name,student.middle_initial,student.last_name,student.student_number,student.year_level,student.year_section].join(' ').toLowerCase().includes(searchParams.q.toLowerCase()))).sort((a,b)=>displayName(a).localeCompare(displayName(b),'en',{sensitivity:'base'}))
