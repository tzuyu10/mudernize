...
export default async function Page({searchParams}:{searchParams:{batch?:string;q?:string;error?:string;message?:string;created?:string}}){
 ...
 return <div className="space-y-6 page-stack">
  ...
  <div className="table-wrap student-tally-table"><table><thead><tr><th>Student</th><th>Batch</th><th>Year &amp; Section</th><th>Access</th><th>Pending</th>{dutyTypes.map(type=><th key={type}>{displayLabel(type)}</th>)}<th>Required</th><th>Actions</th></tr></thead><tbody>{rows.map(student=>{const values=Object.fromEntries(dutyTypes.map(type=>[type,categoryTotal(student.user_id,type)]));const grandTotal=dutyTypes.reduce((sum,type)=>sum+values[type].required,0),studentName=displayName(student);return <tr key={student.user_id} className={student.is_active===false?'inactive-user':''}><td><strong>{studentName}</strong><br/><span className="muted">{student.student_number}</span></td><td>{student.batch}</td><td><strong>{student.year_section||'Not set'}</strong><br/><small className="muted">{student.year_level} year</small></td><td><span className={`status-chip ${student.is_active===false?'is-archived':'is-active'}`}>{student.is_active===false?'Suspended':'Active'}</span></td><td>{(registrations||[]).filter(row=>row.student_id===student.user_id&&row.status==='pending').length}</td>{dutyTypes.map(type=><td key={type}><strong className="number-text">{values[type].required}</strong><small className="muted block">{values[type].completed} completed · {values[type].registered} registered · {values[type].remaining} available</small><TallyAdjustmentForm userId={student.user_id} studentName={studentName} dutyType={type} currentRequired={values[type].required} registered={values[type].registered}/></td>)}<td><strong className="themeValue text-xl">{grandTotal}</strong></td><td><StudentAccountManager student={student}/></td></tr>})}</tbody></table></div>{!rows.length&&<p className="muted">No students match your filters.</p>}
 </div>
}
