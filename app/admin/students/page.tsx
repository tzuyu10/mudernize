...
export default async function Page({searchParams}:{searchParams:{batch?:string;q?:string;error?:string;message?:string;created?:string}}){
 ...
 const [{data:batchConfigs},{data:students,error},{data:registrations},{data:adjustments,error:adjustmentError},{data:resetRequests}]=await Promise.all([
  ...
  supabase.from('users').select('user_id,student_number,first_name,last_name,year_level,batch,is_active').eq('role','student').order('last_name'),
