...
export default async function Page({searchParams:searchParamsPromise}:{searchParams:Promise<SearchParams>}) {
 ...
 const filtered=(data||[]).filter(row=>{
  ...
  return [student?.first_name,student?.middle_initial,student?.last_name,student?.student_number,student?.batch,student?.year_level,student?.year_section].filter(Boolean).join(' ').toLowerCase().includes(search)
 })
 ...
 return <div className="space-y-5 page-stack">
  <div className="page-heading"><p className="eyebrow">CLINICAL HEAD WORKSPACE</p><h1>Review registrations</h1><p className="muted text-sm">Search students, filter requests, review evidence, and update duty status.</p></div>
  ...
  <div className="verification-results">
   {rows.map(row=><article key={row.registration_id} className="dashboardCard verification-card p-6">
    <div className="verification-card-head"><div><h2 className="font-semibold">{displayName(row.users||{})}</h2><p className="muted text-xs mt-1">{row.users?.student_number} · {row.users?.batch} · {row.users?.year_section||row.users?.year_level+' year'}</p></div><span className="badge">{displayLabel(row.status)}</span></div>
