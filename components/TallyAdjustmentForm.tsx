...
export default function TallyAdjustmentForm({userId,studentName,dutyType,currentRequired,registered}:{userId:string;studentName:string;dutyType:DutyType;currentRequired:number;registered:number}){
 ...
 const canDecrease=removable>=multiplier
 return <details ref={details} className="tally-editor"><summary className="adminAction">＋ / − Modify</summary><form action={adjustStudentTally}>
  <input type="hidden" name="user_id" value={userId}/><input type="hidden" name="duty_type" value={dutyType}/><input type="hidden" name="change_kind" value={changeKind}/>
  ...
  {dutyType==='unexcused'&&<label>Repeat rotation ratio<select name="ratio" value={ratio} onChange={event=>{const next=Number(event.target.value);setRatio(next);if(changeKind==='decrease'&&removable<next)setChangeKind('increase')}}><option value="3">1:3</option><option value="6">1:6</option></select></label>}
  ...
  <div className="tally-modal-actions"><button type="button" className="adminSecondary" onClick={()=>details.current?.removeAttribute('open')}>Cancel</button><ConfirmButton className={changeKind==='decrease'?'dangerButton':'primary'} message={`${changeKind==='increase'?'Add':'Remove'} ${total} ${displayLabel(dutyType)} duties ${changeKind==='increase'?'to':'from'} ${studentName}'s tally using the 1:${multiplier} rule?`}>{changeKind==='increase'?'Add to Tally':'Remove from Tally'}</ConfirmButton></div>
 </form></details>
}
