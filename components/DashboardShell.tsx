...
const adminLinks:[string,string,IconName][]=[['/admin','Dashboard','home'],['/admin/students','My students','students'],['/admin/schedule','Duty schedules','calendar'],['/admin/verification','Verification','check'],['/admin/announcements','Announcements','bell']]

export default function DashboardShell({children,admin,profile}:{children:React.ReactNode;admin:boolean;profile:any}){
 const [collapsed,setCollapsed]=useState(false),[mobileOpen,setMobileOpen]=useState(false)
 const pathname=usePathname(),batch:Batch=admin?'Admin':profile.batch,links=admin?adminLinks:studentLinks
 const batchLogo=admin?'/logos/slcn-logo.png':`/logos/${String(batch).toLowerCase()}.png`
 ...
 return <div className={`${styles.shell} ${styles[batch.toLowerCase()]}`} data-collapsed={collapsed}>
  {mobileOpen&&<button className={styles.scrim} aria-label="Close menu" onClick={()=>setMobileOpen(false)}/>}
  <aside className={`${styles.sidebar} ${mobileOpen?styles.mobileOpen:''}`}>
   <button className={styles.logoRow} onClick={()=>mobileOpen?setMobileOpen(false):setCollapsed(!collapsed)} aria-label={mobileOpen?'Close sidebar':collapsed?'Expand sidebar':'Collapse sidebar'} aria-expanded={!collapsed}><span className={styles.logoMark}><img src="/logos/mudernize-logo.svg" alt=""/></span><strong>MUD<span>ernize</span></strong><span className={styles.collapseHint} aria-hidden="true">‹</span></button>
