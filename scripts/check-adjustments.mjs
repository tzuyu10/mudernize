...
const [profiles,resets,tallies,batches,users,registrations,tallyVersion]=await Promise.all([
...
if(profiles.error||resets.error||tallies.error||batches.error||users.error||registrations.error||tallyVersion.error||tallyVersion.data!==2){
 console.error('Database adjustments are not installed. Run database/003_product_adjustments.sql through database/010_exact_tally_decrease.sql in order in Supabase SQL Editor.')
 process.exitCode=1
